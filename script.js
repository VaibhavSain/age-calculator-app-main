...
function ageCalc() {
	let today = new Date();
    let inputDay = new Date(document.getElementById('input-day').value);
    let inputMonth = new Date(document.getElementById('input-month').value);
    let inputYear = new Date(document.getElementById('input-year').value);
	let birthMonth, birthDate, birthYear;
	let birthDetails = {
		date: inputDay.getDate(),
		month: inputMonth.getMonth() + 1,
		year: inputYear.getFullYear()
	}
